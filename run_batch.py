...
# Find candidate resumes across the repository
files = list(Path(".").glob("*.pdf")) + list(Path(".").glob("*.docx")) + list(Path(".").glob("*.txt"))
files += list(Path("data").glob("**/*.pdf")) if Path("data").exists() else []
files = [f for f in set(files) if "report" not in f.name.lower() and f.name != "requirements.txt"]

if not files:
    print("No resume files found (.pdf, .docx, or .txt). Add candidate resumes to test.")
    exit()

print(f"2. Auditing {len(files)} candidate profiles...")
records = []
...
for file in files:
    ...
    # Job cosine match
    sims = cosine_similarity(rec_tfidf.transform([cleaned]), job_tfidf_vectors).flatten()
    top_idx = sims.argmax()
...
out_file = "SmartHire_Batch_Rankings.xlsx"
df.to_excel(out_file, index=False)

print("\n" + "=" * 85)
print(f"SUCCESS: Evaluated {len(df)} candidate profiles.")
print(f"Full Excel report saved to: {out_file}")
print("=" * 85)
print(df[["Filename", "Shortlist Probability (%)", "Verdict", "Cohort Percentile", "Primary Domain"]].to_string(index=False))
